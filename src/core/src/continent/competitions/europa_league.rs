...
impl EuropaLeague {
    ...
    pub fn conduct_draw(
        ...
        // Generate group stage fixtures (6 matchdays, Thursdays)
        self.matches.clear();
        let year = date.year();
        let matchday_dates = [
    ...
    pub fn generate_knockout_fixtures(&mut self, year: i32) {
        ...
        info!(
            "Europa League R16: {} ties, {} matches scheduled",
    ...
    pub fn get_club_points(&self, club_id: u32) -> f32 {
        ...
        for group in &self.groups {
            if let Some(row) = group.rows.iter().find(|r| r.team_id == club_id) {
                return 5.0 + row.points as f32 * 1.5;
            }
        }
